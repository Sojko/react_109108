...
const Main = () => {
  ...
  const saveUserPhrase = (data) => {
    setSearchWord(data);
  };

  const movies = searchWord ? filterMovies(allMovies, searchWord) : allMovies;

  return (
    <div className="main">
      <Container>
        <Header title="Wyszukiwarka filmów - aplikacja grupy 5 przedmiotu Wykorzystywanie wzorców w technologiach internetowych" />
        <Bar getUserPhrase={saveUserPhrase} />
